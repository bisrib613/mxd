...
php senderspintax.php %%c %%d %%a %%b betixreader.xyz 587 tls
...
For /F "usebackq tokens=1-4  delims=," %%a in (datasource\%csv%.csv) do (
start /min cmd /k "php sender.php %%c %%d %%a %%b betixreader.xyz 587 tls && exit"
timeout /t 20 /nobreak
)
goto end
:4
...
cd %real_path% 
For /F "usebackq tokens=1-6  delims=," %%a in (datasource\%csv%.csv) do (
php sender.php %%c %%d %%a %%b betixreader.xyz 587 tls
timeout /t 3 /nobreak
php scrapblog.php %%e %%f 
php C:\laragon\www\indexinblog\index.php suntik submit %%e
timeout /t 3 /nobreak
